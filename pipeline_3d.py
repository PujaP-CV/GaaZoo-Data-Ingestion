"""Rupali's pipeline: select items from catalog, convert to 3D via Meshy, save GLB locally, update catalog."""
import requests
from pathlib import Path
from typing import List, Dict, Any

from config import MESHY_API_KEY, DIR_3D
from catalog_db import init_db, get_items_for_conversion, get_item_by_asin, update_conversion_result, update_conversion_failed
from meshy_client import convert_image_to_3d
...
def run_3d_pipeline(limit: int = 5) -> List[Dict[str, Any]]:
    """
    Select items that have 2D but no 3D, convert each via Meshy, save GLB to data/3d/{asin}.glb, update catalog.
    Returns list of results: { asin, status, glb_path? }.
    """
        ...
        if status == "succeeded" and glb_url:
            ...
            if _download_glb(glb_url, glb_path):
                update_conversion_result(
                    asin, glb_path=str(glb_path), conversion_status="succeeded", meshy_task_id=task_id
                )
...
def run_3d_single(asin: str, image_path: str) -> Dict[str, Any]:
    """
    Convert a single image to 3D and update the catalog item.
    image_path: local file path to the image to convert (e.g. from image_paths[user_selected_index]).
    Returns { asin, status, glb_path?, error? }.
    """
    if not MESHY_API_KEY:
        raise ValueError("MESHY_API_KEY is not set in .env")
    init_db()
